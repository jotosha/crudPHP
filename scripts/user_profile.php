...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(!empty($user_firstname) && !empty($user_lastname) && !empty($user_email) && !empty($gender) && !empty($intro) && !empty($user_image)){
            $query_send= "INSERT INTO profile(user_id, firstname, lastname, email, gender, intro, image, date_time) VALUE('{$id}', '{$user_firstname}' , '{$user_lastname}' , '{$user_email}' , '{$gender}', '{$intro}', '{$user_image}', now()) ";
            
            $send = mysqli_query($connection, $query_send);
            
            if(!$send){
                die("DTATA WAS NOT INSERTED . " . mysqli_error($connection));
            }
            ...
            $msg = "<div class='alert alert-danger alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Fields cannot be empty.
                </div>";
