...
<?php 
    ...
    if(isset($_POST['submit'])){
        ...
        if(!empty($firstname) && !empty($lastname) && !empty($email) && !empty($pass_word)){
            if($count > 0){
                $error1 = "<div class='alert alert-success alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>User with this Email already exists.
                </div>";
            }else{
                ...
                if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
                    $error2 = "<div class='alert alert-success alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Email is invalid.
                </div>";
                }
                
                if(!preg_match("/^[a-zA-Z]*$/", $f_name)){
                    $error3 = "<div class='alert alert-success alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Only letters are allowed for Firstname.
                </div>";
                }
                
                if(!preg_match("/^[a-zA-Z]*$/", $l_name)){
                    $error4 = "<div class='alert alert-success alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Only letters are allowed for Lastname.
                </div>";
                }
                ...
                if((preg_match("/^[a-zA-Z]*$/", $f_name)) && (preg_match("/^[a-zA-Z]*$/", $l_name)) && (preg_match("/^\S*(?=\S{7,15})(?=\S*[a-z])(?=\S*[A-Z])(?=\S*[\d])\S*$/", $pass_word)) && (filter_var($email, FILTER_VALIDATE_EMAIL))){
            ...
            if(empty($firstname)){
                $error3 = "<div class='alert alert-danger alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>First name field cannot be blank.
                </div>";
            }
            if(empty($lastname)){
                $error4 = "<div class='alert alert-danger alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Last name field cannot be blank.
                </div>";
            }
            if(empty($email)){
            $error2 = "<div class='alert alert-danger alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Email field cannot be blank.
                </div>";
            }
            if(empty($pass_word)){
            $error5 = "<div class='alert alert-danger alert-dismissable'>
                  <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Password field cannot be blank.
                </div>";
            }
        }
